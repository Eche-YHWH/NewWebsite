            ...
            <div>
              <div className="rounded-[28px] bg-gradient-to-b from-sky-200/70 to-amber-100/40 p-6">
                <div className="rounded-[22px] border border-black/10 bg-white/80 p-4 shadow-sm">
                  <img src={imageSrc} alt={imageAlt} className="w-full rounded-[18px] object-cover" />
                </div>
              </div>
            </div>

            <div>
              <p className="text-xs font-semibold tracking-widest text-black/45">{eyebrow}</p>
              <h3 className="mt-4 text-5xl font-semibold tracking-tight text-black">{title}</h3>
              <p className="mt-5 max-w-xl text-base text-black/60 sm:text-lg">{text}</p>
